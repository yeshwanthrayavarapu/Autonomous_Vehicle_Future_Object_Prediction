...
def get_times(object_deletion_dict, frame_active_objects, frame_path):
    ...
    for i in frame_active_objects:
        ...
        if present_frame==last_frame :
            last_frame=present_frame+3
...
def get_busdata(frame_path, root_dir):
    ...
    json_file = os.path.join(root_dir, scene, 'bus', file + '_bus_signals.json')
...
def feature_extractor(finres, object_deletion_dict, active_objects,dataset_dir):
    X_distance = []
    X_accx = []
    X_accy = []
    X_speed = []
    ...
    root_dir = os.path.join(dataset_dir,'camera_lidar_semantic_bus/')
